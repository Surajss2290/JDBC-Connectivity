package com.jspiders.CRUD.operations;
	...
	public static void main(String[] args) {
		try {
           ...
           //Execute result
		    ...
		    for(int i=0;i<=3;i++){
		    	if (i==1) {	
		    query3="update emp"+" set ename = ' Harsh '"+"where id="+i;
		    result=statement.executeUpdate(query3);
		    System.out.println("Query ok "+result+" rows affected");
		    	}else if (i==3) {
		    		query3="update emp"+" set ename = ' Tushar '"+"where id="+i;	
		    		result=statement.executeUpdate(query3);
		    		System.out.println("Query ok "+result+" rows affected");
				}
		    }
		    ...
		    System.out.println("-----UPDATED RECORD-----");
		     ...
		     for(int i=0;i<6;i++) {
		    	 if(i==2) {
		    	  query="delete from  emp"+" where id="+i;
		         result=statement.executeUpdate(query);
		         System.out.println("Query ok "+result+" rows affected");
		     } else if (i==5) {
		    	 query="delete from  emp"+" where id="+i;
		     ...
		     while(resultSet.next()) {
               ...
               // IF TABLE IS ALREADY NOT PRESENT THEN IT WILL CREATE A TABLE FIRST THEN INSERT THE RECORD
			    query="create table emp(id int PRIMARY KEY,ename varchar(20),job varchar(20),sal LONGTEXT)";
			    result=statement.executeUpdate(query);
